  ...
  renderRequestTable(requests, selectedIndex, onSelect) {
    ...
    requests.forEach((req, index) => {
      ...
      const $tr = $('<tr>')
        ...
        .html(`
          <td>${index + 1}</td>
          <td><strong>${StringUtils.escapeHtml(req.request.method)}</strong></td>
          <td class="text-truncate" style="max-width:150px" title="${StringUtils.escapeHtml(host)}">${StringUtils.escapeHtml(host)}</td>
          <td class="text-truncate" style="max-width:250px" title="${StringUtils.escapeHtml(req.request.url)}">${StringUtils.escapeHtml(urlPath)}</td>
  ...
  updatePaneContent(pane, tabName, content) {
    ...
    if (tabName === 'hex') {
      ...
      if ($code.length) {
        ...
        $code.text(text);
        $code.attr('class', `hljs language-${lang}`);
        if (typeof hljs !== 'undefined' && text && text !== 'Loading...') {
